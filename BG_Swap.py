...
parser.add_argument('--device', type=str, choices=['cpu', 'cuda'], default='cpu') # 原来是cuda
...
def handle(image_path: str, bgr_path: str, new_bg: str):
    ...
    def writer_hy(img, new_bg, path):
        ...
        new_bg_img.resize(img_size, Image.LANCZOS)
...
def process_image(A, B, Path_input, Path_output):
    # Read the original image
    image = cv2.imread(Path_input)
    height, width = image.shape[:2]

    # Scale the image according to the scaling ratio A
    beta = cv2.resize(image, (int(width * A), int(height * A)))

    # Create a white canvas with the same size as the original image
    alpha = np.ones((height, width, 3), dtype=np.uint8) * 255

    # Determine the position to place the scaled image beta on the canvas alpha
    beta_height, beta_width = beta.shape[:2]

    if B == 0:
        # Lower left corner
        x_offset = 0
        y_offset = height - beta_height
    elif B == 1:
        # Center bottom
        x_offset = (width - beta_width) // 2
        y_offset = height - beta_height
    elif B == 2:
        # Lower right corner
        x_offset = width - beta_width
        y_offset = height - beta_height
    else:
        raise ValueError("B must be 0, 1, or 2.")

    # Place the scaled image beta on the canvas alpha
    alpha[y_offset:y_offset + beta_height, x_offset:x_offset + beta_width] = beta

    # Output the final merged image to Path_output
    cv2.imwrite(Path_output, alpha)
...
if __name__ == '__main__':
    ...
    print("欢迎使用背景替换工具 By Blue16")
    print("注意，该工具会以第一个图片的尺寸为基准，对后面两张图片进行调整！")
    image_path = input("输入人像图片路径：")
    bgr_path = input("输入人像背景图片路径：")
    new_bg = input("输入新背景图片路径：")
    location = int(input("输入图像放置位置:"))
    ratio = float(input("输入缩放比:"))

    # 读取第一张图片以获取其尺寸
    image1 = cv2.imread(image_path)
    height, width = image1.shape[:2]
    image_paths = [image_path,bgr_path,new_bg]
    # 创建一个空列表，用于存储调整大小后的图片
    resized_images = []

    # 图像尺寸转为4的倍数（代码的基本要求）
    width = round_to_multiple_of_four(width)
    height = round_to_multiple_of_four(height)
    ...
    # 创建processed文件夹，如果它不存在
    output_folder = 'processed'
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # 保存调整大小后的图片到processed文件夹
    for i, resized_image in enumerate(resized_images):
        ...
        if i < 2:
            # 调整后两个图像的相对位置
            # process_image(ratio, location, output_path, output_path)
            process_image(ratio, location, output_path, output_path)

    print("开始处理")
    new_location = handle(output_folder+"/image1.jpg", output_folder+"/image2.jpg", output_folder+"/image3.jpg")
    print(f"处理完成！保存路径为{new_location}".replace("content/","content\\"))